...
async def get_category(offset: int = 0, limit: int = 100, db: AsyncSession = Depends(get_db)):
    db_category = await news.get_categories(db, offset, limit)
    return {
        "code": 200,
        "message": "success",
        "data": db_category,
...
async def get_list(category_id: int = Query(..., alias="categoryId"),   # alias: 给请求url中的参数映起别名
    ...
    news_list = await news.get_list_by_category_id(category_id, offset, page_size, db)
    total = await news.list_count(category_id, db)
    # 是否还有更多记录可以加载
...
async def get_detail(id: int, db: AsyncSession = Depends(get_db)):
    news_detail = await news.get_detail(id, db)
    if not news_detail:
        raise HTTPException(status_code=404, detail="当前新闻不存在")

    views = await news.increase_news_views(id, db)
    # 为保证可用性,只是记录日志
    if not views:
        logger.error(f"当前新闻: {id} 浏览量加1失败")

    # 获取当前新闻的关联新闻/推荐
    related_news = await news.get_related_news(news_detail.category_id, id, 5, db)
